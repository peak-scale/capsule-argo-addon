...
require (
	...
	// Argo CD requires the Glob interface removed in glob v1.0.0.
	...
	// Kubernetes 0.36 requires the structured-merge-diff/v6 schema API.
...
replace (
	...
	// Keep Kubernetes libraries aligned with Argo CD 3.5.x (kubectl 0.37 changes RunCreate).
